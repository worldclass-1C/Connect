...
Функция ВыполнитьЗапрос(Запрос) Экспорт
	...
	ПараметрыЗапроса.Вставить("URL", Запрос.БазовыйURL  + Запрос.ОтносительныйURL);
	...
	Иначе		
		...
		Если ПараметрыЗапроса.ОписаниеОшибки.Служебное = "" Тогда
			Попытка
				...
				ИначеЕсли ПараметрыЗапроса.ИмяЗапроса = "cataloggyms" или ПараметрыЗапроса.ИмяЗапроса = "gymlist" Тогда
...
Процедура ПолучитьСписокСетей(ПараметрыЗапроса)
	...
	|	Сети.Код КАК Код,
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(Сети.ПрограммаЛояльности) КАК ПрограммаЛояльности,
	|	Сети.МаскаНомераТелефона КАК МаскаНомераТелефона,
	|	Сети.Валюта КАК Валюта,
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(Сети.ВидСети) КАК ВидСети
	|ИЗ
	|	Справочник.Сети КАК Сети
	|ГДЕ
	|	НЕ Сети.ПометкаУдаления
	|	И Сети.ВидСети = &ВидСети
	|УПОРЯДОЧИТЬ ПО
	|	Код";
	
	Если ДанныеЗапроса.Свойство("appName") Тогда
		пЗапрос.УстановитьПараметр("ВидСети", Перечисления.ВидыСетей[ ДанныеЗапроса.appName]);
		НаименованиеВыводитьВместеСВидомСети	= Ложь;
	Иначе
		пЗапрос.Текст	= СтрЗаменить(пЗапрос.Текст, "И Сети.ВидСети = &ВидСети", "" );
		НаименованиеВыводитьВместеСВидомСети	= Истина;
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураСети.Вставить("type", Выборка.ВидСети);
		...
		НаименованиеСети	= ?(ЯзыкПриложения="ru", Выборка.НаименованиеRu, Выборка.Наименование);
		НаименованиеСети	= ?(НаименованиеВыводитьВместеСВидомСети, Выборка.ВидСети + " " + НаименованиеСети, НаименованиеСети);		 
		СтруктураСети.Вставить("name", НаименованиеСети);
	...
	Иначе
		...
		Иначе
			...
			Если Выборка.ТипЗапроса = Перечисления.ТипыHTTPЗапросов.GET  Тогда
				ТелоЗапроса			= "";
				ПараметрыЗапросаИзURL	= СтрЗаменить(ПараметрыЗапроса.URL,  ОбщегоНазначенияПовторноеИспользование.ПолучитьБазовыйURL(),  "");
			Иначе
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ВыполнятьВФоне Тогда
					...
					ФЗ	= РаботаСHTTP.ВыполнитьЗапросВФоне(СтруктураПодключения, Заголовки, ТелоЗапроса, Адрес, ПараметрыЗапросаИзURL);			
				...
				Иначе
					...
					ЗапросHTTP = Новый HTTPЗапрос(Выборка.URL + Выборка.Приемник + ПараметрыЗапросаИзURL, Заголовки);				
