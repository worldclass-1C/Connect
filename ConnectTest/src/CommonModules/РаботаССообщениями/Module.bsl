...
Функция НовоеСообщение(ДанныеСообщения) Экспорт
	...
	СообщениеОбъект.Приоритет		= ?(ДанныеСообщения.Свойство("Приоритет"), ДанныеСообщения.Приоритет, "");
...
Процедура ОтправитьPush(Параметры) Экспорт
	...
	Если Параметры.ТокенУстройства <> "" Тогда
		...
		Уведомление.Данные	= РаботаСHTTP.СтруктуруВПараметрыHTTPЗапроса(СтруктураJSON);
