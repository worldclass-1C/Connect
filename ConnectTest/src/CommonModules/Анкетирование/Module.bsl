...
Процедура ОбновитьПревьюТабличныйВопрос(Вопросы,Ответы,ТипТабличногоВопроса,Форма,ИмяТаблицыПревью,Ключ) Экспорт
	...
	ИначеЕсли ТипТабличногоВопроса = Перечисления.ТипыТабличныхВопросов.ПредопределенныеОтветыВСтрокахИКолонках Тогда
		...
		РезультирующаяТаблицаСервер.Колонки.Добавить(ИмяКолонкиБезНомера + "1",ТаблицаВопросы[0].ValueType,ТаблицаВопросы[0].Формулировка);
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(ИмяКолонкиБезНомера + "1",ТаблицаВопросы[0].ValueType,ИмяТаблицыПревью));
		...
		Для инд = 1 По ОтветыВКолонках.Количество() Цикл
			...
			РезультирующаяТаблицаСервер.Колонки.Добавить(ИмяКолонкиБезНомера + XMLСтрока(инд + 1),ТаблицаВопросы[2].ValueType,ОтветыВКолонках[инд-1].Ответ);
			ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(ИмяКолонкиБезНомера + XMLСтрока(инд + 1),ТаблицаВопросы[2].ValueType,ИмяТаблицыПревью));
	...
	Для инд = 1 По ТаблицаВопроса.Колонки.Количество() Цикл
		...
		Если (ТипТабличногоВопроса = Перечисления.ТипыТабличныхВопросов.ПредопределенныеОтветыВКолонках)Тогда
			Если инд = 1 Тогда
				Элемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
			Иначе	
				Элемент.СвязьПоТипу = Новый СвязьПоТипу("Элементы." + ИмяТаблицы + ".CurrentData." + ИмяТаблицы + "_Колонка_1");
				СвязиПараметровВыбора = Новый Массив();
				СвязиПараметровВыбора.Добавить(Новый СвязьПараметраВыбора("Отбор.Владелец","Элементы." + ИмяТаблицы + ".CurrentData." + ИмяТаблицы + "_Колонка_1"));
...
Процедура УдалитьВопросыШаблонаАнкеты(СсылкаВладелец) Экспорт
	...
	Попытка
		...
		Если Не РезультатЗапроса.Пустой() Тогда
			...
			МассивСсылок = РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Ref");
...
Функция ТаблицаДоступныхРеспондентуАнкет(Респондент) Экспорт
	...
	Запрос.Текст =
		...
		|	ВЫБОР
		|		КОГДА АнкетыПоДействующимЗапросам.Ссылка ЕСТЬ NULL
		|			ТОГДА ДействующиеОпросы.Ссылка
		...
		|		ПО ДействующиеОпросы.Ссылка = АнкетыПоДействующимЗапросам.Опрос
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
