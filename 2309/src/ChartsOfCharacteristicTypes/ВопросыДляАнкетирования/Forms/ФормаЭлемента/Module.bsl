...
Процедура ОткрытьФормуЭлементаСправочникаВопросыОтветовАнкет(Элемент,РежимДобавления)
	...
	Если Не РежимДобавления Тогда
		ТекущиеДанные = Элементы.ТаблицаВариантыОтветов.CurrentData;
		...
		СтруктураПараметров.Вставить("Порядок",ТекущиеДанные.РеквизитДопУпорядочивания);
	КонецЕсли;
...
Процедура УстановитьТипОтвета()
	...
	Для каждого ЗначениеПеречисления Из Метаданные.Перечисления.ТипыОтветовНаВопрос.ЗначенияПеречисления Цикл
		...
		Иначе
			//блокировка типов, которые пока не реализованы
			EnumVal = Metadata.Enums.ТипыОтветовНаВопрос.EnumValues;
			Если ЗначениеПеречисления = EnumVal.Булево 
				или ЗначениеПеречисления = EnumVal.Текст Тогда
				Продолжить
			КонецЕсли;
