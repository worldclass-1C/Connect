...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТекущийРежимЗапуска() = РежимЗапускаКлиентскогоПриложения.ОбычноеПриложение Тогда
		...
		мНаличиеПраваАдминистрирования = УправлениеПользователямиСервер.ЕстьПравоАдминистрированияПользователей();
...
Процедура ПроверитьСинхронизациюДанныхПользователя()
	...
	ИначеЕсли НЕ Привилегированный.ПользовательСуществует(Объект.ИдентификаторПользователяИБ) Тогда
		РассогласованиеДанных = 2;
	ИначеЕсли Привилегированный.РассинхронизацияИмениПользователя(Объект.Ссылка) Тогда
...
&НаКлиенте
Процедура ПолученОтветОк(Результат, Параметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.ОК Тогда
		Результат = УдалитьПользователяИБНаСервере();
		Если Результат.Статус Тогда
			ОбновитьИнтерфейс();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Пользователь информационной базы успешно удален!'"));
		Иначе
...
Процедура ПередЗаписью(Отказ)
	...
	Если РассогласованиеДанных = 1 Или РассогласованиеДанных = 2 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Есть рассогласование учетных данных.'"),,,, Отказ);
		Возврат;
	...
	Если ПравоАдминистрирования Тогда
		...
		Если НЕ ПроверитьРолиНазначены() Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Пользователю информационной базы не установлено ни одной роли.'"),,,, Отказ);			
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Объект.Ссылка)
		   И Привилегированный.ПользовательСуществует(СокрЛП(Объект.Код)) Тогда
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru = 'Пользователь с именем ""%1"" уже существует.'"),
										СокрЛП(Объект.Код) );
			
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщенияОбОшибке,,"Код",, Отказ);
		КонецЕсли;
	КонецЕсли;
		...
		Если Не Отказ Тогда
			...
			Если РассогласованиеДанных = 1 ИЛИ РассогласованиеДанных = 2 Тогда
				Идентификатор = УправлениеПользователямиСервер.ЗаписатьПользователяИБ(
										СокрЛП(ТекущийОбъект.Ссылка.ПолучитьОбъект().Наименование),
...
Процедура ПользовательОСНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	//+Степовая ошибка модальности
	//Результат = ОткрытьФормуМодально("Справочник.Пользователи.Форма.ФормаВыбораПользователяОС");
	//
	//Если ТипЗнч(Результат) = Тип("Строка") Тогда
	//	ПользовательОС = Результат;
	//КонецЕсли;
	ОткрытьФорму("Справочник.Пользователи.Форма.ФормаВыбораПользователяОС" ,,,,,, Новый ОписаниеОповещения("ОбработкаПослеВыбораПользователя",ЭтотОбъект));
#КонецЕсли
	
КонецПроцедуры
//++Степовая
&НаКлиенте
Процедура ОбработкаПослеВыбораПользователя(Результат,ДополнительныеПараметры) Экспорт
	Если ТипЗнч(Результат) = Тип("Строка") Тогда
		ПользовательОС = Результат;
	КонецЕсли;
КонецПроцедуры
...
Процедура УдалитьПользователяИБ(Команда)
	...
	ОписаниеОповещения	= Новый ОписаниеОповещения("ПолученОтветОк", ЭтаФорма);
	
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Внимание! Будет удален пользователь информационной базы 
...
Процедура УдалитьВыбранныеРоли()
	...
	Если СтрокиКУдалениюСпросить.Количество() <> 0 Тогда
		Для каждого ЭлКоллекции Из Параметры.ДополнительныеПараметры.СтрокиКУдалениюСпросить Цикл
			Параметры.ДополнительныеПараметры.СтрокиКУдалению.Добавить(ЭлКоллекции);
		КонецЦикла; 
	КонецЕсли; 
